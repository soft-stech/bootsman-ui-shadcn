...
<template>
  <Primitive :as="as" class="data-disabled:opacity-75" :data-disabled="disabled ? true : undefined">
    <div class="flex gap-4">
      <div ref="selectorRef" :class="colorPickerSelectorVariants({ size })" :style="selectorStyle">
        <div class="relative h-full w-full rounded-md" data-color-picker-background>
          <div
            ...
            :class="
              cn(
                'ring-background absolute size-4 -translate-x-1/2 -translate-y-1/2 cursor-pointer rounded-full ring-2 data-disabled:cursor-not-allowed',
                ui?.selectorClass
              )
            "
      ...
      <div ref="trackRef" :class="colorPickerTrackVariants({ size })" data-color-picker-track>
        <div
          ...
          :class="
            cn(
              'ring-background absolute size-4 -translate-x-[4px] -translate-y-1/2 transform cursor-pointer rounded-full ring-2 data-disabled:cursor-not-allowed rtl:translate-x-[4px]',
              ui?.trackClass
            )
          "
